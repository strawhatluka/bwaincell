...
class Reminder {
    static async createReminder(guildId, channelId, message, time, frequency = 'once', dayOfWeek = null, userId, targetDate, dayOfMonth, month) {
        const nextTrigger = this.calculateNextTrigger(time, frequency, dayOfWeek, targetDate, dayOfMonth, month);
    ...
    static async updateNextTrigger(reminderId) {
        ...
        if (reminder.frequency === 'once') {
            ...
            const nextTrigger = this.calculateNextTrigger(reminder.time, reminder.frequency, reminder.day_of_week ?? null, null, reminder.day_of_month ?? null, reminder.month ?? null);
            const { data } = await supabase_1.default
                .from('reminders')
