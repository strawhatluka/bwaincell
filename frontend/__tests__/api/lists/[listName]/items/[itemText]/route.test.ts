...
describe('/api/lists/[listName]/items/[itemText] DELETE', () => {
  ...
  it('removes the item', async () => {
    ...
    const res = await DELETE(makeReq(), {
      params: { listName: 'Groceries', itemText: 'milk' },
    });
  ...
  it('returns 401 when no session', async () => {
    ...
    const res = await DELETE(makeReq(), { params: { listName: 'g', itemText: 'x' } });
  ...
  it('returns 404 when item not found', async () => {
    ...
    const res = await DELETE(makeReq(), { params: { listName: 'g', itemText: 'missing' } });
    expect(res.status).toBe(404);
  });
