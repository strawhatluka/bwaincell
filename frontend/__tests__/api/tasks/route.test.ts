...
describe('/api/tasks', () => {
  ...
  describe('GET', () => {
    it('returns tasks for authenticated user', async () => {
      (prisma.task.findMany as jest.Mock).mockResolvedValue([{ id: 1, description: 'Task 1' }]);
    ...
    it('returns 500 on prisma error', async () => {
      (prisma.task.findMany as jest.Mock).mockRejectedValue(new Error('db fail'));
      const res = await GET(new NextRequest('http://localhost/api/tasks'));
  ...
  describe('POST', () => {
    ...
    it('creates task with dueDate', async () => {
      ...
      expect(res.status).toBe(201);
      expect((prisma.task.create as jest.Mock).mock.calls[0][0].data.dueDate).toBeInstanceOf(Date);
    });
